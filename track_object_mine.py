...
while True:
	...
	rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) # Dlib은 RGB 포맷 필요
	...
	if tracker is None:
		...
		if len(detections) > 0:
			...
			# filter out weak detections and ensure the label matches
			if conf > args["confidence"] and label == args["label"]:
				# compute the (x, y)-coordinates of the bounding box
				box = detections[0, 0, i, 3:7] * np.array([w, h, w, h])
				(startX, startY, endX, endY) = box.astype("int")

				# construct a dlib rectangle object and start the tracker
				tracker = dlib.correlation_tracker()
				rect = dlib.rectangle(startX, startY, endX, endY)
				tracker.start_track(rgb, rect) # Dlib은 RGB 이미지 사용
	...
	# the object (추적 중 로직 + 재탐지 로직 추가)
	else:

		tracker.update(rgb)
		
	
		confidence_score = tracker.get_score() 
		
		
		if confidence_score < 5.0: 
			
			tracker = None
			label = "LOST / REDETECTING" # 상태 표시
			
			
			cv2.putText(frame, label, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)
